...
public class GenericMDS_NotSoGood extends AbstractModulePanel {
	...
	public GenericMDS_NotSoGood() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("88dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("113px"),
				FormFactory.RELATED_GAP_COLSPEC }, new RowSpec[] {
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("91px"),
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("84dlu"),
	...
	protected VersionQueuePanel_NotSoGood getVersionQueuePanel() {
		if (versionQueuePanel == null) {
			versionQueuePanel = new VersionQueuePanel_NotSoGood(
					this.templateModule);
		}
	...
	protected void initialize() throws ModuleException {
		...
		add(getVersionQueuePanel(), new CellConstraints(
				"2, 4, 3, 3, fill, fill"));
		add(getEmail(), new CellConstraints(2, 8, 5, 1));
		add(getMemoryInputPanel(), new CellConstraints(6, 6,
		...
		try {
			...
			getMemoryInputPanel().setTemplateNode(
					this.templateModule.getTemplateNodes().get("MinMem"));
