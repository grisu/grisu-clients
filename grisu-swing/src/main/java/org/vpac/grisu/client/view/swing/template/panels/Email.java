...
public class Email extends JPanel implements TemplateNodePanel {
	...
	static final Logger myLogger = Logger.getLogger(Email.class.getName());
	...
	public Email() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("45dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("46dlu"),
				FormFactory.RELATED_GAP_COLSPEC }, new RowSpec[] {
				FormFactory.RELATED_GAP_ROWSPEC, FormFactory.DEFAULT_ROWSPEC,
				FormFactory.DEFAULT_ROWSPEC, FormFactory.RELATED_GAP_ROWSPEC,
				FormFactory.DEFAULT_ROWSPEC, FormFactory.RELATED_GAP_ROWSPEC }));
		add(getNotifyMeWhenCheckBox(), new CellConstraints(8, 2,
		...
		add(getErrorLabel(), new CellConstraints(2, 3, 7, 1,
				CellConstraints.FILL, CellConstraints.DEFAULT));
		add(getNotifyWhenJobStartsCheckBox(), new CellConstraints(6, 2,
				CellConstraints.RIGHT, CellConstraints.TOP));
		add(getLabel(), new CellConstraints(2, 2, 3, 1, CellConstraints.LEFT,
				CellConstraints.TOP));
		//
	}
	...
	private void calculateDisplayingEmailField() {
		...
		if (getNotifyMeWhenCheckBox().isSelected()
				|| getNotifyWhenJobStartsCheckBox().isSelected()) {
	...
	public void reset() {
		...
		// doesn't need to be implemented because templateNodeUpdated is
		// overwriteen
	...
	protected void setTemplateAttributes() {
		...
		if (getNotifyWhenJobStartsCheckBox().isSelected()) {
			this.templateNode.getElement().setAttribute(
					Constants.SEND_EMAIL_ON_JOB_START_ATTRIBUTE_KEY, "true");
		} else {
			this.templateNode.getElement().setAttribute(
					Constants.SEND_EMAIL_ON_JOB_START_ATTRIBUTE_KEY, "false");
		}

		if (getNotifyMeWhenCheckBox().isSelected()) {
			this.templateNode.getElement().setAttribute(
					Constants.SEND_EMAIL_ON_JOB_END_ATTRIBUTE_KEY, "true");
		} else {
			this.templateNode.getElement().setAttribute(
					Constants.SEND_EMAIL_ON_JOB_END_ATTRIBUTE_KEY, "false");
		}
	...
	public void setTemplateNode(TemplateNode node)
		...
		if (this.templateNode.hasProperty(TemplateNode.LAST_USED_PARAMETER)) {
			...
			String last = null;
			try {
				last = this.templateNode.getTemplate().getEnvironmentManager()
						.getHistoryManager().getEntries(
								EMAIL_HISTORY_STRING_KEY).get(0);
			} catch (Exception e) {
				// no previous entry
			}
		...
		if (this.templateNode.hasProperty(TemplateNode.LAST_USED_PARAMETER)) {
			...
			try {
				checkedEmailOnEnd = this.templateNode.getTemplate()
						...
						.getEntries(EMAIL_ON_JOB_END_HISTORY_CHECKED_KEY)
						.get(0);
			} catch (Exception e) {
				// no previous entry
			...
			try {
				checkedEmailOnStart = this.templateNode.getTemplate()
						.getEnvironmentManager().getHistoryManager()
						.getEntries(EMAIL_ON_JOB_START_HISTORY_CHECKED_KEY)
						.get(0);
			} catch (Exception e) {
	...
	public void templateNodeUpdated(TemplateNodeEvent event) {
		...
		if (event.getEventType() == TemplateNodeEvent.TEMPLATE_PROCESSED_INVALID) {
			...
			if (getNotifyMeWhenCheckBox().isSelected()
					&& (input != null && EmailUtils.isValid(input))) {
				templateNode.getTemplate().getEnvironmentManager()
						.getHistoryManager().addHistoryEntry(
								EMAIL_HISTORY_STRING_KEY, input, new Date(), 1);
				templateNode.getTemplate().getEnvironmentManager()
						.getHistoryManager().addHistoryEntry(
								EMAIL_ON_JOB_END_HISTORY_CHECKED_KEY, "true",
								new Date(), 1);
			} else {
				templateNode.getTemplate().getEnvironmentManager()
						.getHistoryManager().addHistoryEntry(
								EMAIL_ON_JOB_END_HISTORY_CHECKED_KEY, "false",
								new Date(), 1);
			}
			...
			if (getNotifyWhenJobStartsCheckBox().isSelected()
				...
				templateNode.getTemplate().getEnvironmentManager()
						.getHistoryManager().addHistoryEntry(
								EMAIL_ON_JOB_START_HISTORY_CHECKED_KEY, "true",
								new Date(), 1);
			} else {
				templateNode.getTemplate().getEnvironmentManager()
						.getHistoryManager().addHistoryEntry(
								EMAIL_ON_JOB_START_HISTORY_CHECKED_KEY,
								"false", new Date(), 1);
			}
