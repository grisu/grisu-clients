...
public class PreviewPanel extends JPanel {
	...
	public PreviewPanel() {
		...
		setLayout(new FormLayout(new ColumnSpec[] { ColumnSpec
				.decode("163px:grow(1.0)") }, new RowSpec[] {
				RowSpec.decode("165px:grow(1.0)"),
				FormFactory.RELATED_GAP_ROWSPEC, FormFactory.DEFAULT_ROWSPEC }));
		//
		add(getLabel(), new CellConstraints(1, 3, CellConstraints.FILL,
	...
	private PreviewPanelInsert findHandlerPanel(GrisuFileObject file) {
		...
		for (Class panelClass : previewPanelClasses) {
			...
			try {
				...
				// this means the above specified class is not available or does
	...
	public void previewFile(GrisuFileObject file) {
		...
		try {
			...
			Utils
					.showErrorMessage(file.getFileSystemBackend()
							.getEnvironmentManager(), this,
							"couldNotGetOrCacheFile", e);
		}
	...
	public void refresh() {
		if (insert != null) {
			try {
				...
				Utils.showErrorMessage(file.getFileSystemBackend()
						.getEnvironmentManager(), this,
						"couldNotGetOrCacheFile", e);
			}
		}
	...
	private void setFileInfo() {
		if (file != null) {
			...
			String lastModified = new Date(file.getLastModified(false))
					.toString();
			getLabel().setText(
					"<html><body><b>Size:</b> " + sizeString
