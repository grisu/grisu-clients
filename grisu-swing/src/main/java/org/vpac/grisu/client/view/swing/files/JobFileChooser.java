...
public class JobFileChooser extends JPanel {
	...
	static final Logger myLogger = Logger.getLogger(JobFileChooser.class
			.getName());
	...
	private void initialize() {
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_COLSPEC, new ColumnSpec("100dlu"),
		...
		add(getRefreshButton(), new CellConstraints(4, 2,
				CellConstraints.RIGHT, CellConstraints.DEFAULT));
		add(getCreateDirectoryButton(), new CellConstraints(2, 2,
	...
	public void setJob(GrisuJobMonitoringObject job) throws FileSystemException {
		...
		rootDir.refresh();
		if (rootDir != null)
			myLogger.debug("RootDir for job \"" + job.getName() + "\": "
					+ rootDir.getURI().toString());
		else {
			myLogger.error("RootDir for job \"" + job.getName()
					+ "\" is null. Not setting anything...");
	...
	private void showPopup(MouseEvent e) {
		...
		try {
			if (selectedFileForMenu.getFileSystemBackend().getSite().equals(
