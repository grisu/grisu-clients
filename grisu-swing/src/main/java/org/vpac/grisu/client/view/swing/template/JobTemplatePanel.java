...
/**
 * Creates a panel that renders all necessary fields to enable the user to
 * create a job for a specific jsdl template.
 * 
 * It sepereates all fields into tabs according to the modules that are in a
 * {@link JsdlTemplate}.
 * 
...
public class JobTemplatePanel extends JPanel implements JsdlTemplateListener {
	...
	static final Logger myLogger = Logger.getLogger(JobTemplatePanel.class
			.getName());
	...
	public void setTemplate(JsdlTemplate template) throws ModuleException {
		...
		for (String moduleName : template.getModules().keySet()) {
			...
			// TODO is that good enough. I reckon. this method will always
			// return something like: package org.vpac....
			// String packageNameModule =
			// module.getClass().getPackage().toString().substring(8);
			String packageNameModule = module.getClass().toString()
					.substring(6);
			// String classNamePanel = packageNameModule.replace(".model.",
			// ".view.swing.")+"."+moduleName;
			String classNamePanel = packageNameModule.replace(".model.",
					".view.swing.");
	...
	public void submissionExceptionOccured(JsdlTemplateEvent event,
		...
		// not important. The panel get's notified via the TemplateNode if
		// something is wrong.
