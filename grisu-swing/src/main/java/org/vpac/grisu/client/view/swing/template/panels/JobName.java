...
public class JobName extends JPanel implements TemplateNodePanel {
	...
	static final Logger myLogger = Logger.getLogger(JobName.class.getName());
	...
	public JobName() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("43dlu"),
		...
		add(getErrorLabel(), new CellConstraints(2, 1, 3, 1,
	...
	private void autosuggest() {
		...
		Set<String> allJobs = this.templateNode.getTemplate()
				.getEnvironmentManager().getJobManager().getAllJobnames(false);
	...
	protected JButton getSuggestButton() {
		if (suggestButton == null) {
			...
			suggestButton
					.setToolTipText("Suggests a jobname that is not already taken.");
	...
	public void setTemplateNode(TemplateNode node)
		...
		// throw new
		// TemplateNodePanelException("JobName template node has a multiplicity other than one.");
		// }
		...
		if (this.templateNode.hasProperty(NOSUGGESTBUTTON_PROPERTY)) {
			...
			add(getTextField(), new CellConstraints(2, 3, 3, 1,
					CellConstraints.FILL, CellConstraints.BOTTOM));
		} else {
			add(getSuggestButton(), new CellConstraints(4, 4,
					CellConstraints.FILL, CellConstraints.BOTTOM));
			add(getTextField(), new CellConstraints(2, 4, CellConstraints.FILL,
					CellConstraints.BOTTOM));
		}
