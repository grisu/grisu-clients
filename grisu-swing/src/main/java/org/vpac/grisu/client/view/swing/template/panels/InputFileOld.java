...
public class InputFileOld extends JPanel {
	...
	public InputFileOld() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("9dlu"),
	...
	private void fillComboBox() {
		...
		if (useHistory
				&& historyManager != null
				&& historyManager.getEntries(this.templateNode.getName())
						.size() > 0) {
			for (String entry : historyManager.getEntries(this.templateNode
					.getName())) {
	...
	public void reset() {
		...
		if (lastUsed)
			historyManager
					.addHistoryEntry(this.templateNode.getName() + "_"
							+ TemplateNode.LAST_USED_PARAMETER,
							getSiteFileChooserDialog().getCurrentDirectory()
									.toString());
	...
	public void setTemplateNode(TemplateNode node)
		...
		if (this.templateNode.getOtherProperties().containsKey(
			...
			// String url =
			// historyManager.getEntries(this.templateNode.getName()+"_"+TemplateNode.LAST_USED_PARAMETER).get(0);
			// URI uri = null;
		...
		if (this.templateNode.getOtherProperties().containsKey(
			...
			if (!TemplateNode.NON_MAP_PARAMETER.equals(maxString)) {
				...
				historyManager.setMaxNumberOfEntries(this.templateNode
						.getName(), maxValues);
			}
