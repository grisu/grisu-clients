...
public class MDSCommandLine extends JPanel implements TemplateNodePanel,
		SubmissionObjectListener {
	...
	public MDSCommandLine() {
		...
		layout = new FormLayout(new ColumnSpec[] {
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("30dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("42dlu"),
				...
				FormFactory.RELATED_GAP_ROWSPEC, FormFactory.DEFAULT_ROWSPEC,
				RowSpec.decode("top:1dlu"), FormFactory.DEFAULT_ROWSPEC,
		...
		add(getExecutableCombobox(), new CellConstraints(2, 6, 3, 1,
				CellConstraints.FILL, CellConstraints.FILL));
		add(getArgumentsCombobox(), new CellConstraints(6, 6, 3, 1,
				CellConstraints.FILL, CellConstraints.FILL));
		add(getRequiredLabel(), new CellConstraints(8, 4,
	...
	private void fillArgumentsCombobox() {
		...
		if (useHistory
				...
				&& historyManager.getEntries(
						EXECUTABLE_HISTORY_PREFIX
								+ historyManagerKeyForThisNode).size() > 0) {
			String exeValue = historyManager.getEntries(
					EXECUTABLE_HISTORY_PREFIX + historyManagerKeyForThisNode)
					.get(0);
	...
	private String getHelpText(String key) {
		...
		if (help == null || "".equals(help)) {
			help = this.templateNode.getInfoMap().get(
					TemplateNode.DEFAULT_HELP_ATTRIBUTE_NAME);
			if (help == null || "".equals(help)) {
		...
		if (label == null) {
			...
			label.addMouseListener(new MouseAdapter() {
				public void mouseClicked(final MouseEvent arg0) {
					...
					String selectedApplication = (String) executableModel
							.getSelectedItem();

					HelpDialog hd = new HelpDialog(
							getHelpText(selectedApplication));
					hd.setVisible(true);
				}
			});
	...
	public void reset() {
		...
		if (useHistory && value != null && !"".equals(value)) {
			historyManager.addHistoryEntry(historyManagerKeyForThisNode, value,
					new Date());
			historyManager.addHistoryEntry(EXECUTABLE_HISTORY_PREFIX
					+ historyManagerKeyForThisNode, exeValue);
		}
	...
	public void setExternalSetValue(String value) {
		...
		if (value != null) {
			...
			if (index == -1) {
				...
				getExecutableCombobox().setSelectedItem(
	...
	public void setTemplateNode(TemplateNode node)
		...
		try {
			// this bit is a little hairy. We need to be sure that the CommonMDS
			// module is there
			TemplateModule submissionObjectHolderModule = node.getTemplate()
					.getModule("Common");
			if (submissionObjectHolderModule == null) {
				submissionObjectHolderModule = node.getTemplate().getModule(
						"CommonMDS");
			}
			if (submissionObjectHolderModule == null) {
				throw new TemplateNodePanelException(
						"Could not find a \"Common\" or \"CommonMDS\" module for the MDSCommandline template panel.");
			}

			submissionObjectHolder = (SubmissionObjectHolder) submissionObjectHolderModule;
		} catch (Exception e) {
			throw new TemplateNodePanelException(
					"Can't render this template for an unknow reason.", e);
		}
		if (submissionObjectHolder == null)
			throw new TemplateNodePanelException(
					"Can't create submission holder object. Can't render this template.");
		...
		if (!this.templateNode.hasProperty(TemplateNode.HIDE_DESCRIPTION)
			...
			add(getScrollPane(), new CellConstraints(2, 2, 7, 1,
					CellConstraints.FILL, CellConstraints.FILL));
		} else {
			// don't know
		...
		try {
			submissionObjectChanged(submissionObjectHolder
					.getCurrentSubmissionObject());
		...
		add(getArgumentsLabel(), new CellConstraints(6, 8, 3, 1,
				CellConstraints.RIGHT, CellConstraints.DEFAULT));

		fillArgumentsCombobox();

		if (this.templateNode.getInfoMap().size() > 0) {
			add(getLabel(), new CellConstraints(4, 8, CellConstraints.RIGHT,
