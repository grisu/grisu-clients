...
public class GenericMDS extends AbstractModulePanel {
	...
	public GenericMDS() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("102dlu"),
				FormFactory.RELATED_GAP_COLSPEC }, new RowSpec[] {
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("56dlu"),
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("51dlu"),
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("35dlu"),
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("20dlu"),
				FormFactory.RELATED_GAP_ROWSPEC, FormFactory.DEFAULT_ROWSPEC,
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("52dlu"),
				FormFactory.RELATED_GAP_ROWSPEC }));
		add(getJobName(), new CellConstraints(2, 2, CellConstraints.FILL,
		...
		add(getEmail(), new CellConstraints(2, 10, 3, 3, CellConstraints.FILL,
				CellConstraints.FILL));
		add(getVersion(), new CellConstraints(2, 4, 3, 1, CellConstraints.FILL,
				CellConstraints.FILL));
	...
	protected void initialize() throws ModuleException {
		try {
			...
			this.template.getTemplateNodes().get(
					TemplateTagConstants.HOSTNAME_TAG_NAME)
					.setTemplateNodeValueSetter(getSubmissionLocation());
			this.template.getTemplateNodes().get(
					TemplateTagConstants.VERSION_TAG_NAME)
					.setTemplateNodeValueSetter(getVersion());
			this.template.getTemplateNodes().get(
					TemplateTagConstants.EXECUTIONFILESYSTEM_TAG_NAME)
					.setTemplateNodeValueSetter(executionFileSystem);
			...
			getVersion().setTemplateNode(
					this.templateModule.getTemplateNodes().get(
							TemplateTagConstants.VERSION_TAG_NAME));
			getSubmissionLocation().setTemplateNode(
					this.templateModule.getTemplateNodes().get(
							TemplateTagConstants.HOSTNAME_TAG_NAME));
