...
public class GenericAuto extends AbstractModulePanel {
	...
	public GenericAuto() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("82dlu"),
				FormFactory.RELATED_GAP_COLSPEC,
				ColumnSpec.decode("43dlu:grow(1.0)"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("60dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("59dlu"),
				FormFactory.RELATED_GAP_COLSPEC }, new RowSpec[] {
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("48dlu"),
				FormFactory.RELATED_GAP_ROWSPEC,
				RowSpec.decode("78dlu:grow(1.0)"),
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("60dlu"),
				FormFactory.RELATED_GAP_ROWSPEC, FormFactory.DEFAULT_ROWSPEC,
				FormFactory.RELATED_GAP_ROWSPEC }));
		add(getJobName(), new CellConstraints(2, 2, 5, 1, CellConstraints.FILL,
				CellConstraints.FILL));
		add(getCpus(), new CellConstraints(8, 2, 1, 3, CellConstraints.FILL,
				CellConstraints.FILL));
		add(getMemoryInputPanel(), new CellConstraints(8, 8,
				CellConstraints.FILL, CellConstraints.FILL));
		add(getEmail(), new CellConstraints(2, 8, 5, 1, CellConstraints.FILL,
				CellConstraints.FILL));
		add(getVersion(), new CellConstraints(2, 6, 3, 1, CellConstraints.FILL,
				CellConstraints.FILL));
		add(getWallTime(), new CellConstraints(6, 6, 3, 1));
		add(getGridResourceSuggestionPanel(), new CellConstraints(2, 4, 5, 1,
				CellConstraints.FILL, CellConstraints.FILL));
	}
	...
	protected void initialize() throws ModuleException {
		...
		try {
			getGridResourceSuggestionPanel().setTemplateNode(
