...
public class ExecutionFileSystem extends JPanel implements TemplateNodePanel, ValueListener {
	
	static final Logger myLogger = Logger.getLogger(ExecutionFileSystem.class.getName());
	...
	public ExecutionFileSystem() {
		super();
		setBorder(new TitledBorder(null, "ExecutionFileSystem", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, null, null));
		setLayout(new FormLayout(
			...
			new RowSpec[] {
				...
				FormFactory.RELATED_GAP_ROWSPEC}));
		add(getExecutionFileSystemTextField(), new CellConstraints(2, 2, CellConstraints.FILL, CellConstraints.DEFAULT));
		//
	}
	...
	throws TemplateNodePanelException {
		...
		if ( getSubmissionLocationPanel() != null && getSubmissionLocationPanel().getCurrentExecutionFileSystem() != null ) {
			setExternalSetValue(getSubmissionLocationPanel().getCurrentExecutionFileSystem());
		}
	}
	...
	public void addValueListener(ValueListener v) {
		// not needed here because this panel will never actively change the value. It's all done in the SubmissionLocation panel for now
	}
	...
	protected JTextField getExecutionFileSystemTextField() {
		if (executionFileSystemTextField == null) {
			...
			executionFileSystemTextField.setHorizontalAlignment(SwingConstants.CENTER);
