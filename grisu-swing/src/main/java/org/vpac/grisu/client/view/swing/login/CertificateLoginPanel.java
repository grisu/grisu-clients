...
public class CertificateLoginPanel extends JPanel {
	...
	public CertificateLoginPanel() {
		...
		add(getLoginButton(), new CellConstraints(5, 6, CellConstraints.RIGHT,
				CellConstraints.BOTTOM));
		add(cancelButton(), new CellConstraints(3, 6, CellConstraints.RIGHT,
				CellConstraints.BOTTOM));
	...
	private Person getUser() {
		...
		try {
			user = new Person(ClientPropertiesManager.getClientConfiguration(),
					username + " Grisu certificateLogin user");
		} catch (Exception e) {
			try {
	...
	private void login() {
		...
		try {
			// serviceInterface =
			// LoginHelpers.localProxyLogin(getPasswordField().getPassword(),
			// loginPanelHolder.getLoginParams());
			serviceInterface = LoginManager.login(null, getPasswordField()
					.getPassword(), null, null, loginPanelHolder
					.getLoginParams());
		} catch (LoginException e1) {
			Utils.showErrorMessage(getUser(), CertificateLoginPanel.this,
					"loginError", e1);
		...
		// // calling this method only to check whether the backend can get a
		// proxy
		// // with them
		// serviceInterface.login(loginPanelHolder.getLoginParams().getMyProxyUsername(),
		// loginPanelHolder.getLoginParams().getMyProxyPassphrase());
		// } catch (Exception e2) {
		// Utils.showErrorMessage(getUser(), CertificateLoginPanel.this,
