...
public class GrisuFilePanel extends JPanel implements FileChooserParent {
		...
		if (leftPanel == null) {
			...
			leftPanel.setLayout(new FormLayout(
				...
				new RowSpec[] {
					...
					FormFactory.RELATED_GAP_ROWSPEC}));
			leftPanel.add(getLeftSiteFileChooserPanel(), new CellConstraints(2, 2, 3, 1, CellConstraints.FILL, CellConstraints.FILL));
			leftPanel.add(getLeftCopyButton(), new CellConstraints(4, 4, CellConstraints.RIGHT, CellConstraints.DEFAULT));
		}
		return leftPanel;
	}
		...
		if (righPanel == null) {
			...
			righPanel.setLayout(new FormLayout(
				...
				new RowSpec[] {
					...
					FormFactory.DEFAULT_ROWSPEC,
					FormFactory.RELATED_GAP_ROWSPEC}));
			righPanel.add(getHolderPanel(), new CellConstraints(2, 2, 3, 1, CellConstraints.FILL, CellConstraints.FILL));
			righPanel.add(getRightCopyButton(), new CellConstraints(2, 4, CellConstraints.LEFT, CellConstraints.DEFAULT));
			righPanel.add(getRightSideTypeComboBox(), new CellConstraints(4, 4, CellConstraints.FILL, CellConstraints.DEFAULT));
		}
		return righPanel;
	}
	...
	protected JPanel getHolderPanel() {
		if (holderPanel == null) {
			...
			holderPanel.add(getRightSiteFileChooserPanel(), getRightSiteFileChooserPanel().getName());
		...
		if (leftCopyButton == null) {
			...
			leftCopyButton.addActionListener(new ActionListener() {
				public void actionPerformed(final ActionEvent e) {
					...
					GrisuFileObject[] sourceFiles = getLeftSiteFileChooserPanel().getSelectedFiles();
					try {
						getLeftSiteFileChooserPanel().setBusy(true);
						getRightSiteFileChooserPanel().setBusy(true);
						GrisuFilePanel.this.setCursor(Cursor
...
//										sourceFiles, targetDirectory, true);
						...
						// this means that the user cancelled an upload/download
						myLogger.debug("File transfer exception. Probably because user cancelled it.");
						// doing nothing here
					} catch (Exception e1) {
...
//										sourceFiles, targetDirectory, true);
	...
	protected JComboBox getRightSideTypeComboBox() {
		if (rightSideTypeComboBox == null) {
			rightSideTypeComboBox = new JComboBox(new String[]{FILE_PANEL, PREVIEW_PANEL});
			rightSideTypeComboBox.setMinimumSize(new Dimension(100, 0));
			rightSideTypeComboBox.addItemListener(new ItemListener() {
				public void itemStateChanged(final ItemEvent e) {
					switchPanels((String)getRightSideTypeComboBox().getSelectedItem());
				}
			});
		}
