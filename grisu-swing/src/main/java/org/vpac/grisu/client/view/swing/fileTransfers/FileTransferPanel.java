...
public class FileTransferPanel extends JPanel implements FileTransferListener {
	...
	.getLogger(FileTransferPanel.class.getName());
	...
	public void initialize(FileTransferManager ftm) {
		...
		String[] propertyNames = new String[] { "sourcesString", "targetDirectory", "transferStatusString" };
		String[] columnLabels = new String[] { "Sources", "Target", "Status" };

		TableFormat<FileTransfer> tf = GlazedLists.tableFormat(FileTransfer.class, propertyNames, columnLabels);
		SortedList<FileTransfer> sortedTransferList = new SortedList<FileTransfer>(
				this.fileTransferManager.getTransferList(),	new FileTransferComparator());

//		// add filtering to the table
//		FilterList<FileTransfer> textFilteredIssues = new FilterList<FileTransfer>(
//				sortedJobList, new TextComponentMatcherEditor(getFilterField(),new FileTransferFilterator()));

//		jobModel = new EventTableModel<GrisuJobMonitoringObject>(textFilteredIssues, tf);
		tableModel = new EventTableModel<FileTransfer>(sortedTransferList, tf);
		selectionModel = new EventSelectionModel<FileTransfer>(sortedTransferList);
			...
			public void run() {
				...
				try {
					for (FileTransfer transfer : selectionModel.getSelected()) {
						...
						ftdd.getFileTransferDetailsPanel().setFileTransfer(transfer);
						setPanelBusy(false);
					...
					myLogger.error(e);
					e.printStackTrace();
//					Utils.showErrorMessage(em, GlazedJobMonitorPanel.this,	"couldNotFindJobDirecotory", e);
				} finally {
					setPanelBusy(false);
				}
			}
