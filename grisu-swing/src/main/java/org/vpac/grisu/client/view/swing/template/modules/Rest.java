...
public class Rest extends AbstractModulePanel {
	...
	protected void initialize() throws ModuleException {
		...
		for (String name : this.templateModule.getTemplateNodes().keySet()) {
			...
			TemplateNode node = this.templateModule.getTemplateNodes()
					.get(name);
			...
			if (node.getType().contains(".")) {
				...
				panelClassName = "org.vpac.grisu.client.view.swing.template.panels."
						+ node.getType();
			}
			...
			try {
				...
				myLogger.error("Could not create template node panel for "
			...
			try {
				...
				myLogger.error("Could not initialize template node panel for "
						+ node.getType() + ": " + e.getLocalizedMessage());
				throw new ModuleException(this.templateModule, e);
			}
		...
		// ok. no we've got all panel created. Let's throw them into the parent
		// panel.
		int y = 0;
		for (TemplateNodePanel panel : templateNodePanels.values()) {
			...
			this.getParentPanel()
					.add(panel.getTemplateNodePanel(), constraints);
		}
