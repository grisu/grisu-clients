...
public class InputInteger_OLD extends JPanel implements TemplateNodePanel {
	...
	public InputInteger_OLD() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_COLSPEC, new ColumnSpec("34dlu"),
		...
		add(getErrorLabel(), new CellConstraints(2, 2, CellConstraints.FILL,
				CellConstraints.DEFAULT));
		add(getRequiredLabel(), new CellConstraints(4, 2,
			...
			slider.addChangeListener(new ChangeListener() {
				public void stateChanged(final ChangeEvent e) {
					...
					getValueLabel().setText(
							new Integer(getSlider().getValue()).toString());
	...
	public void setTemplateNode(TemplateNode node)
		...
		if (useHistory)
			;
		// historyNode =
		// this.templateNode.getTemplate().getEnvironmentManager().getHistoryManager().getHistoryNode(this.templateNode.getName());
		...
		if (SPINNER_PANEL.equals(renderMode)) {
			...
			if (this.templateNode.getOtherProperties().containsKey(
				...
				((JSpinner.DefaultEditor) getSpinner().getEditor())
			...
			try {
				...
				myLogger.error("Could not set default value for InputInteger "
						+ this.templateNode.getName());
			}

		} else if (SLIDER_PANEL.equals(renderMode)) {

			add(getSlider(), new CellConstraints(2, 4));
			add(getValueLabel(), new CellConstraints(4, 4,
					CellConstraints.RIGHT, CellConstraints.DEFAULT));

			Integer min = 0;
			try {
				min = Integer.parseInt(this.templateNode.getOtherProperties()
						.get("min"));
			} catch (NumberFormatException e) {
				myLogger
						.error("Could not get minimum for slider. Setting to 0.");
			}
			Integer max = 100;
			try {
				max = Integer.parseInt(this.templateNode.getOtherProperties()
						.get("max"));
			} catch (NumberFormatException e) {
			...
			try {
				delta = Integer.parseInt(this.templateNode.getOtherProperties()
						.get("delta"));
			} catch (Exception e) {
				myLogger
						.error("Could not get max for slider. Don't setting it.");
			}
		...
		if (COMBOBOX_PANEL.equals(renderMode)
				|| (prefills != null && prefills.length > 0)) {
			...
			if (useHistory && historyNode != null
					&& historyNode.getNumberOfEntries() > 0) {
			...
			// render textfield
			add(getTextField(), new CellConstraints(2, 4, 3, 1,
		...
		this.setBorder(new TitledBorder(null, templateNode.getName(),
