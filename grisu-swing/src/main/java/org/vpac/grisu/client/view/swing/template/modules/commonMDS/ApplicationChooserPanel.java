...
public class ApplicationChooserPanel extends JPanel implements
	...
	static final Logger myLogger = Logger
			.getLogger(ApplicationChooserPanel.class.getName());
	...
	private void displayMatchingQueues() {
		...
		SubmissionLocation oldQueue = (SubmissionLocation) getQueueList()
				.getSelectedValue();
		...
		if (currentVersionObject == null) {
			possibleSubmissionLocations = currentApplicationObject
					.getSubmissionLocationsForSite(selectedSite);
		} else {
			possibleSubmissionLocations = currentVersionObject
					.getSubmissionLocationsForSite(selectedSite);
		}
	...
	private void displayMatchingSites(ApplicationObject appObj) {
		...
		Set<String> availableSites = appObj
				.getActualAvailableSites(environmentManager.getDefaultFqan());
	...
	private void displayMatchingSites(VersionObject versionObj) {
		...
		Set<String> availableSites = versionObj
				.getSitesWhereThisVersionIsAvailable();
	...
	/**
	 * Returns the values for submission location and application/version as
	 * currently selected by the user. If information is missing, null is
	 * returned.
	 * 
	...
	public SubmissionObject getCurrentlySelectedSubmissionObject() {
		...
		SubmissionLocation location = (SubmissionLocation) getQueueList()
				.getSelectedValue();
		...
		if (currentVersionObject == null) {
			...
			myLogger.debug("Selected non-version ApplicationObject "
					+ application.getName() + " and submission location: "
			...
			myLogger.debug("Selected VersionObject " + application.getName()
					+ ", Version: " + currentVersionObject.getCurrentVersion()
					+ " and submission location: " + location.getLocation());
		...
		if (queueList == null) {
			...
			queueList.addMouseListener(new MouseAdapter() {
				public void mouseClicked(final MouseEvent e) {
					commonMdsModule
							.submissionObjectChanged(getCurrentlySelectedSubmissionObject());
				}
			});
		}
		return queueList;
	...
	public void initialize(CommonMDS module) {
		...
		root = new DefaultMutableTreeNode("Available applications");
		...
		this.executionFileSystem = module.getTemplateNodes().get(
				"ExecutionFileSystem");
		...
		this.executionFileSystem
				.setTemplateNodeValueSetter(executionFileSystemSetter);
		...
		allGridApps = serviceInterface.getAllAvailableApplications(
				DtoStringList.fromStringColletion(environmentManager
	...
	public void valueChanged(TreeSelectionEvent e) {
		...
		if (nodeInfo instanceof ApplicationObject) {
			...
			if (selectedApplication.getChildCount() == 0) {
				...
				VersionObject[] versions = appObj
						.getVersionsForAllSites(environmentManager
								.getDefaultFqan());
