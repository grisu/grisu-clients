...
public class VersionQueuePanel_NotSoGood extends JPanel implements
		ActionListener, SubmissionObjectHolder {

	static final Logger myLogger = Logger
			.getLogger(VersionQueuePanel_NotSoGood.class.getName());
	...
	public VersionQueuePanel_NotSoGood(TemplateModule templateModule) {
		...
		try {
			this
					.addSubmissionObjectListener((SubmissionObjectListener) templateModule);
		} catch (Exception e) {
			throw new RuntimeException(
					"Can't create VersionQueuePanel because the templateModule this is connected to ("
							+ templateModule.getModuleName()
							+ ") is not implementing the SubmissionObjectListener interface.");
		}
		...
		this.executionFileSystem = templateModule.getTemplateNodes().get(
				"ExecutionFileSystem");

		this.versionNode = templateModule
				.getTemplateNodes()
				.get(
						org.vpac.grisu.client.model.template.modules.GenericMDS.VERSION_TEMPLATE_TAG_NAME);
		this.hostnameNode = templateModule
				.getTemplateNodes()
				.get(
						org.vpac.grisu.client.model.template.modules.GenericMDS.HOSTNAME_TEMPLATE_TAG_NAME);
		this.application = templateModule.getTemplate().getApplicationName();

		this.anyModeSupported = versionNode.getOtherProperties().containsKey(
				ANY_MODE_TEMPLATETAG_KEY);
		this.defaultModeSupported = versionNode.getOtherProperties()
				.containsKey(DEFAULT_MODE_TEMPLATETAG_KEY);
		this.exactModeSupported = versionNode.getOtherProperties().containsKey(
				EXACT_MODE_TEMPLATETAG_KEY);

		// set startup mode
		String startUpMode = versionNode.getOtherProperties().get(
				STARTUP_MODE_KEY);
		...
		try {
			this.exactModeVersion = versionNode.getOtherProperties().get(
					EXACT_MODE_TEMPLATETAG_KEY);
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("33dlu"),
				FormFactory.RELATED_GAP_ROWSPEC,
				RowSpec.decode("29dlu:grow(1.0)"),
				FormFactory.RELATED_GAP_ROWSPEC }));

		add(getVersionPanel(), new CellConstraints(2, 2, 3, 1,
				CellConstraints.FILL, CellConstraints.FILL));
		add(getSubLocPanel(), new CellConstraints(2, 4, 3, 1,
				CellConstraints.FILL, CellConstraints.FILL));
		//
		this.infoObject = new ApplicationInfoObject(em, application,
				this.currentMode);

		for (String version : em.getAllAvailableVersionsForApplication(
				application, em.getDefaultFqan())) {
			versionModel.addElement(version);
		}

		if (this.exactModeVersion != null
				&& em.getAllAvailableVersionsForApplication(application,
						em.getDefaultFqan()).contains(this.exactModeVersion)) {
	...
	private void changeSelectionOfQueues(String site) {
		...
		if (site != null && !"".equals(site)) {
			...
			for (SubmissionLocation subLoc : infoObject
					.getCurrentlyPossibleSubmissionLocationsForSite(site)) {
	...
	private void fillSiteCombobox() {
		...
		if (infoObject != null
				&& infoObject.getCurrentlyPossibleSubmissionLocations() != null) {
			...
			if (oldSubLoc != null) {
				if (infoObject.getCurrentlyPossibleSites().contains(
						oldSubLoc.getSite())) {
		...
		if (siteComboBox == null) {
			...
			siteComboBox.addItemListener(new ItemListener() {
				public void itemStateChanged(final ItemEvent e) {
					...
					changeSelectionOfQueues(((String) (siteModel
							.getSelectedItem())));
				}
			});
		}
	...
	protected JPanel getSubLocPanel() {
		if (subLocPanel == null) {
			...
			subLocPanel.setLayout(new FormLayout(new ColumnSpec[] {
					ColumnSpec.decode("37dlu"),
					FormFactory.RELATED_GAP_COLSPEC,
					ColumnSpec.decode("33dlu:grow(1.0)") }, new RowSpec[] {
					FormFactory.DEFAULT_ROWSPEC,
					FormFactory.RELATED_GAP_ROWSPEC,
					FormFactory.DEFAULT_ROWSPEC }));
			subLocPanel.setBorder(new TitledBorder(null, "Submission location",
					TitledBorder.DEFAULT_JUSTIFICATION,
					TitledBorder.DEFAULT_POSITION, null, null));
			subLocPanel.add(getSiteLabel(), new CellConstraints(1, 1,
					CellConstraints.RIGHT, CellConstraints.DEFAULT));
			subLocPanel.add(getQueueLabel(), new CellConstraints(1, 3,
					CellConstraints.RIGHT, CellConstraints.DEFAULT));
	...
	private void recalculateApplicationInfoObject() {
		if (!ignoreQueueChanges) {
			SubmissionLocation tempLoc = (SubmissionLocation) (queueModel
					.getSelectedItem());
			if (tempLoc != null) {
				try {
					// check any mode, if, then set the currect version first
					if (currentMode == ApplicationInfoObject.ANY_VERSION_MODE) {
						String tempVersion = infoObject
								.getRecommendedVersionForSubmissionLocation(
										tempLoc, em.getDefaultFqan());
				...
				if (currentMode == ApplicationInfoObject.ANY_VERSION_MODE
					...
					// String temp =
					// infoObject.getRecommendedVersionForSubmissionLocation(tempLoc,
					// em.getDefaultFqan());
	...
	private void switchMode(int mode) {
		try {
			...
			case ApplicationInfoObject.EXACT_VERSION_MODE:
				...
				break;
			default:
				myLogger.error("Can't switch to mode: " + mode
						+ ". Not supported.");
			}
	...
	private void switchToAnyVersionMode() throws ModeNotSupportedException {
		...
		SubmissionLocation tempSubmissionLocation = ((SubmissionLocation) queueModel
				.getSelectedItem());
		siteModel.removeAllElements();
		queueModel.removeAllElements();
		...
		this.currentMode = ApplicationInfoObject.ANY_VERSION_MODE;
		ignoreVersionComboboxItemChanged = true;
		Set<String> allVersions = em.getAllAvailableVersionsForApplication(
				application, em.getDefaultFqan());
	...
	private void switchToExactVersionMode() throws ModeNotSupportedException {
		...
		infoObject.setMode(ApplicationInfoObject.EXACT_VERSION_MODE,
				((String) (versionModel.getSelectedItem())));
		if (exactModeVersion == null || "".equals(exactModeVersion)) {
			...
			if (!infoObject.getAllAvailableVersions()
					.contains(exactModeVersion)) {
				versionModel.removeAllElements();
				versionModel.addElement("Version " + exactModeVersion
						+ " not available.");
				ignoreVersionComboboxItemChanged = false;
			} else {
				ignoreVersionComboboxItemChanged = false;
