...
public class JobStatusPanel extends JPanel implements JsdlTemplateListener {
	...
	public JobStatusPanel() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				ColumnSpec.decode("left:26dlu") }, new RowSpec[] {
				RowSpec.decode("top:14dlu"), RowSpec.decode("17dlu"),
				RowSpec.decode("top:14dlu"), FormFactory.DEFAULT_ROWSPEC,
				RowSpec.decode("39dlu:grow(1.0)"), RowSpec.decode("top:12dlu"),
				RowSpec.decode("11dlu"), FormFactory.RELATED_GAP_ROWSPEC,
				RowSpec.decode("32dlu"), RowSpec.decode("13dlu") }));
		add(getCancelButton(), new CellConstraints(2, 9,
				CellConstraints.CENTER, CellConstraints.TOP));
		add(getScrollPane(), new CellConstraints(2, 5, CellConstraints.FILL,
				CellConstraints.FILL));
		add(getProgressBar(), new CellConstraints(2, 2, CellConstraints.FILL,
				CellConstraints.BOTTOM));
		add(getDeleteCheckBox(), new CellConstraints(2, 7,
				CellConstraints.CENTER, CellConstraints.TOP));
		//
	}
	...
	public void setSubmissionStatus() {
		int newStatus = template.getStatus()
				- JsdlTemplate.STATUS_SUBMISSION_STARTED + noPostProcessors;
		myLogger.debug("Setting status: " + newStatus + "/"
				+ wholeNoOfStepsInSubmission);
	...
	public void templateStatusChanged(JsdlTemplateEvent event) {
		...
		if (template.getStatus() == JsdlTemplate.STATUS_JOB_SUBMISSION_SUCCESSFUL) {
			...
			if (template.getStatus() >= JsdlTemplate.STATUS_JOB_CREATED
					&& noPostProcessors == -1) {
				noPostProcessors = template.getNumberOfPostprocessors();
				myLogger.debug("No. of postprocessors: " + noPostProcessors);
				wholeNoOfStepsInSubmission = JsdlTemplate.STATUS_JOB_SUBMISSION_SUCCESSFUL
						- JsdlTemplate.STATUS_SUBMISSION_STARTED;
				myLogger.debug("Steps without postprocessors: "
						+ wholeNoOfStepsInSubmission);
				wholeNoOfStepsInSubmission = wholeNoOfStepsInSubmission
						+ noPostProcessors;
