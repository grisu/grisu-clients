...
public class DefaultJobFilesPanel extends JPanel {
	...
	public DefaultJobFilesPanel(EnvironmentManager em) {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				FormFactory.RELATED_GAP_ROWSPEC, new RowSpec("top:default"),
				FormFactory.RELATED_GAP_ROWSPEC, new RowSpec("top:default"),
				FormFactory.RELATED_GAP_ROWSPEC }));
		add(getStdoutPanel(), new CellConstraints(2, 2, CellConstraints.FILL,
				CellConstraints.TOP));
		add(getStderrPanel(), new CellConstraints(2, 4, CellConstraints.FILL,
				CellConstraints.TOP));
		//
	}
	...
	public void setJob(GrisuJobMonitoringObject job) {
		...
		try {
			jobDir = em.getFileManager().getFileObject(
					new URI(job.getJobDirectory()));
		} catch (URISyntaxException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		this.fs = new FileSystemListFrontend(jobDir.getFileSystemBackend(),
				jobDir);
		try {
			GrisuFileObject stdout = fs.getFileSystemBackend().getFileObject(
					new URI(job.getStdout()));
			if (stdout != null)
				getStdoutPanel().setFile("stdout", stdout);
			GrisuFileObject stderr = fs.getFileSystemBackend().getFileObject(
					new URI(job.getStderr()));
			if (stderr != null)
