...
public class AddMountPointDialog extends JDialog {
	...
	private JEditorPane getAdvancedHelpEditorPane() {
		if (advancedHelpEditorPane == null) {
			...
			advancedHelpEditorPane.setText("Please enter the root filesystem and a mountpoint to mount a new filesystem. ");
	...
	private JCheckBox getAdvancedConfigCheckBox() {
		if (advancedConfigCheckBox == null) {
			...
			advancedConfigCheckBox.addItemListener(new java.awt.event.ItemListener() {
	...
	private JPanel getSimpleAddMountPointPanel() {
		if (SimpleAddMountPointPanel == null) {
			...
			SimpleAddMountPointPanel.setBorder(BorderFactory.createEtchedBorder(EtchedBorder.RAISED));
			SimpleAddMountPointPanel.add(getSiteComboBox(), gridBagConstraints9);
			SimpleAddMountPointPanel.add(siteLabel, gridBagConstraints10);
			SimpleAddMountPointPanel.add(simpleConfigLabel, gridBagConstraints12);
		}
	...
	private JCheckBox getAutoHomeDirCheckBox() {
		if (autoHomeDirCheckBox == null) {
			...
			autoHomeDirCheckBox.setText("Use your home directory on that filesystem");
			autoHomeDirCheckBox.setEnabled(false);
			autoHomeDirCheckBox.setHorizontalTextPosition(SwingConstants.LEADING);
