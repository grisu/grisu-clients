...
public class MyProxyLoginPanel extends JPanel {
	...
	public MyProxyLoginPanel() {
		...
		setLayout(new FormLayout(new ColumnSpec[] {
				...
				ColumnSpec.decode("0dlu"), ColumnSpec.decode("42dlu"),
				ColumnSpec.decode("left:10dlu"), FormFactory.DEFAULT_COLSPEC,
				ColumnSpec.decode("left:6dlu") }, new RowSpec[] {
				RowSpec.decode("top:9dlu"), FormFactory.DEFAULT_ROWSPEC,
		...
		String defaultUserName = MyProxyServerParams
				.loadDefaultMyProxyUsername();
		...
		add(getCancelButton(), new CellConstraints(4, 11,
				CellConstraints.RIGHT, CellConstraints.DEFAULT));
		add(getLoginButton(), new CellConstraints(6, 11, CellConstraints.RIGHT,
				CellConstraints.DEFAULT));
		if (ProxyLightLibraryManager.prerequisitesForProxyCreationAvailable()) {
			add(getMyproxyInitButton(), new CellConstraints(2, 11,
		...
		if (myproxyInitButton == null) {
			...
			myproxyInitButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					String myproxyServer = MyProxyServerParams
							.getMyProxyServer();
					int myproxyPort = MyProxyServerParams.getMyProxyPort();
					int lifetime_in_seconds = -1; // to allow the user to
													// specify the time
					String allowed_retrievers = null;
					String allowed_renewers = null;
					Map<String, char[]> loginInfo = ProxyLightLibraryManager
							.createMyProxyInitDialog(myproxyServer,
									myproxyPort, lifetime_in_seconds,
									allowed_retrievers, allowed_renewers);
	...
	private Person getUser() {
		...
		try {
			user = new Person(ClientPropertiesManager.getClientConfiguration(),
					username + " Grisu myproxylogin user");
		} catch (Exception e) {
			try {
	...
	private void login() {
		...
		try {
			...
			Utils.showErrorMessage(getUser(), MyProxyLoginPanel.this,
					"loginError", e1);
		...
		// // calling this method only to check whether the backand can get a
		// proxy
		// // with them
		// serviceInterface.login(myProxyUsernameTextField.getText(),
		// passwordField.getPassword());
		// } catch (Exception e2) {
		// Utils.showErrorMessage(getUser(), MyProxyLoginPanel.this,
		// "wrongMyProxyPassphrase", e2);
		// getLoginButton().setEnabled(true);
		// return;
		// }

		if (serviceInterface != null) {
			MyProxyServerParams.saveDefaultMyProxyUsername(params
					.getMyProxyUsername());
