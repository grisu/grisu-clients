...
public class JobFileListWithPreviewPanel extends JPanel implements
				...
				public void actionPerformed(final ActionEvent e) {
					...
					new Thread() {
						public void run() {
							...
							GrisuFileObject[] sourceFiles = getJobFileChooser()
									.getSelectedFiles();
							try {
								JobFileListWithPreviewPanel.this
										.setCursor(Cursor
		...
		if (panel == null) {
			...
			panel.setLayout(new FormLayout(
					new ColumnSpec[] { FormFactory.RELATED_GAP_COLSPEC,
							...
							FormFactory.RELATED_GAP_ROWSPEC }));
			panel.add(getHolderPanel(), new CellConstraints(2, 2, 3, 1,
					CellConstraints.FILL, CellConstraints.FILL));
			panel.add(getCopyButton(), new CellConstraints(2, 4,
					CellConstraints.LEFT, CellConstraints.DEFAULT));
			panel.add(getComboBox(), new CellConstraints(4, 4,
	...
	protected SiteFileChooserPanel getRightSiteFileChooserPanel() {
		if (rightSiteFileChooserPanel == null) {
			...
			rightSiteFileChooserPanel.changeCurrentDirectory(em
