...
public class CandidateHost extends JPanel implements TemplateNodePanel,
	...
	public CandidateHost() {
		setBorder(new TitledBorder(null, "Submission location", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		setLayout(new FormLayout(new ColumnSpec[] {
			...
			new RowSpec[] {
				...
				FormFactory.RELATED_GAP_ROWSPEC,}));
		add(getLblSite(), "2, 1");
		add(getInfoPanel(), "3, 1, 1, 7, fill, fill");
	...
	public void setTemplateNode(TemplateNode node)
		...
		serviceInterface = node.getTemplate().getEnvironmentManager().getServiceInterface();
		registry = GrisuRegistry.getDefault(serviceInterface);

		this.currentFqan = registry.getUserEnvironmentManager().getCurrentFqan(); 
	}
	...
	private void calculateAvailableGridResources() {
		...
		jobProperties.put(JobSubmissionProperty.APPLICATIONVERSION, currentVersion);
		
		SortedSet<GridResource> resources = registry.getUserApplicationInformation(applicationName).getBestSubmissionLocations(jobProperties, currentFqan);
		...
		if (queueComboBox == null) {
			...
			queueComboBox.addItemListener(new ItemListener() {
				public void itemStateChanged(ItemEvent arg0) {
					...
					try {
						currentSubmissionLocation = SubmissionLocationHelpers.createSubmissionLocationString(((GridResource)queueModel.getSelectedItem()));
					} catch (Exception e) {
						e.printStackTrace();
					}
					}
