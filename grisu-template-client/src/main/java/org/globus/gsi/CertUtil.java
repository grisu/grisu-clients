...
public class CertUtil {
	...
	private static Log logger = LogFactory.getLog(CertUtil.class.getName());

	private static I18n i18n = I18n.getI18n("org.globus.gsi.errors",
	 ...
	 * certificate must be in PEM/Base64 format and start with
	 ...
	 * contents must start with "BEGIN CERTIFICATE" line and end with
	 * "END CERTIFICATE" line, and be in PEM/Base64 format.
	 * 
	...
	public static void writeCertificate(OutputStream out, X509Certificate cert)
		...
		PEMUtils.writeBase64(out, "-----BEGIN CERTIFICATE-----",
				Base64.encode(cert.getEncoded()), "-----END CERTIFICATE-----");
	}

	/**
	 * Converts DN of the form "CN=A, OU=B, O=C" into Globus format
	 * "/CN=A/OU=B/O=C".<BR>
	 * This function might return incorrect Globus-formatted ID when one of the
	 * RDNs in the DN contains commas.
	 * 
	...
	/**
	 * Converts DN of the form "CN=A, OU=B, O=C" into Globus format
	 * "/CN=A/OU=B/O=C" or "/O=C/OU=B/CN=A" depending on the
	 * <code>noreverse</code> option. If <code>noreverse</code> is true the
	 * order of the DN components is not reveresed - "/CN=A/OU=B/O=C" is
	 * returned. If <code>noreverse</code> is false, the order of the DN
	 * components is reversed - "/O=C/OU=B/CN=A" is returned. <BR>
	 * This function might return incorrect Globus-formatted ID when one of the
	 * RDNs in the DN contains commas.
	 * 
	 * @param dn
	...
	/**
	 * Converts the specified principal into Globus format. If the principal is
	 * of unrecognized type a simple string-based conversion is made using the
	 * {@link #toGlobusID(String) toGlobusID()} function.
	 * 
	...
	/**
	 * Installs SecureRandom provider. This function is automatically called
	...
	public static boolean isGsi4Proxy(int certType) {
		return (certType == GSIConstants.GSI_4_IMPERSONATION_PROXY
				...
				|| certType == GSIConstants.GSI_4_RESTRICTED_PROXY || certType == GSIConstants.GSI_4_LIMITED_PROXY);
	}
	...
	public static boolean isGsi3Proxy(int certType) {
		return (certType == GSIConstants.GSI_3_IMPERSONATION_PROXY
				...
				|| certType == GSIConstants.GSI_3_RESTRICTED_PROXY || certType == GSIConstants.GSI_3_LIMITED_PROXY);
	}
	...
	public static boolean isGsi2Proxy(int certType) {
		return (certType == GSIConstants.GSI_2_PROXY || certType == GSIConstants.GSI_2_LIMITED_PROXY);
	}
	...
	public static boolean isLimitedProxy(int certType) {
		return (certType == GSIConstants.GSI_3_LIMITED_PROXY
				|| certType == GSIConstants.GSI_2_LIMITED_PROXY || certType == GSIConstants.GSI_4_LIMITED_PROXY);
	}
	...
	public static boolean isIndependentProxy(int certType) {
		return (certType == GSIConstants.GSI_3_INDEPENDENT_PROXY || certType == GSIConstants.GSI_4_INDEPENDENT_PROXY);
	}
	...
	public static boolean isImpersonationProxy(int certType) {
		return (certType == GSIConstants.GSI_3_IMPERSONATION_PROXY
				...
				|| certType == GSIConstants.GSI_2_LIMITED_PROXY || certType == GSIConstants.GSI_2_PROXY);
	}
	 ...
	 * 
	 * @param proxyType
	 *            the proxy type to get the string description of.
	 ...
	 * 
	 * @return true if <I>"org.globus.gsi.version"</I> system property is set to
	 *         "3". Otherwise, false.
	 */
	...
	public static X509CRL loadCrl(String file) throws IOException,
