	...
	public FluentIterationChartPanel() {
		...
		setLayout(new FormLayout(
			...
			new RowSpec[] {
				...
				FormFactory.RELATED_GAP_ROWSPEC}));
		add(getRefreshButton(), new CellConstraints(4, 4, CellConstraints.RIGHT, CellConstraints.DEFAULT));
		add(getScrollPane(), new CellConstraints(2, 2, 3, 1, CellConstraints.FILL, CellConstraints.FILL));
		//
		...
		if (refreshButton == null) {
			...
			refreshButton.addActionListener(new ActionListener() {
				public void actionPerformed(final ActionEvent e) {
					...
					fluentJob.refresh();
					lineChart.setSubtitle("Time: "+SimpleDateFormat.getDateTimeInstance().format(new Date()));
				}
			});
