...
public class PostprocessorFactory {
	...
	public static ArrayList<ElementPostprocessor> createPostprocessors(
			JsdlTemplate template, Element element) {

		ArrayList<ElementPostprocessor> postprocessors = new ArrayList<ElementPostprocessor>();
		String[] postprocessorNames = element.getAttribute("postprocess")
				.split(",");
		...
		for (String postprocessorValue : postprocessorNames) {
			...
			if (startConfigIndex > -1) {
				...
				configText = postprocessorValue.substring(startConfigIndex + 1,
						endConfigIndex);
				postprocessorName = postprocessorValue.substring(0,
						startConfigIndex);
			} else {
			...
			try {
				...
				if (postprocessorName != null && !"".equals(postprocessorName)) {
					...
					if (postprocessorName.indexOf(".") == -1) {
						...
						postprocessorClass = Class
								.forName("org.vpac.grisu.client.model.template.postprocessor."
										+ postprocessorName);
					} else {
						// means custom type
					...
					ElementPostprocessor pp = (ElementPostprocessor) validatorConstructor
							.newInstance(new Object[] { template, element });
					pp.setConfig(configText);
					postprocessors.add(pp);
				}
			} catch (Exception e) {
				myLogger.warn("Could not create postprocessor \""
						+ postprocessorName + "\": " + e.getLocalizedMessage());
				// return null;
			}
		}
