...
public class CommonMDS extends AbstractModule implements SubmissionObjectHolder {
	...
	public static final String[] MODULES_USED = new String[] {
			...
			TemplateTagConstants.APPLICATION_TAG_NAME,
	...
	public void submissionObjectChanged(SubmissionObject so) {
		...
		templateNodes.get("Application").getTemplateNodeValueSetter()
				.setExternalSetValue(so.getCurrentApplicationName());
		templateNodes.get("HostName").getTemplateNodeValueSetter()
				.setExternalSetValue(
						so.getCurrentSubmissionLocation().getLocation());

		String site = template.getEnvironmentManager().lookupSite(
				EnvironmentManager.QUEUE_TYPE,
				so.getCurrentSubmissionLocation().getLocation());
		String fqan = template.getEnvironmentManager().getDefaultFqan();

		// String[] stagingFS =
		// environmentManager.lookupStagingFileSystemsForQueue(so.getLocation().getLocation());

		// MountPoint mp = null;

		// for ( String fs : stagingFS ) {
		// mp =
		// environmentManager.getDefaultMountPointForSiteAndFqan(environmentManager.lookupSite(EnvironmentManager.FILE_URL_TYPE,
		// fs), fqan);
		// mp =
		// environmentManager.getMountPointsForSubmissionLocationAndFqan(so.getLocation().getLocation(),
		// fqan).iterator().next();
		// if ( mp != null )
		// break;
		// }
		...
		// MountPoint mp =
		// environmentManager.getDefaultMountPointForSiteAndFqan(site, fqan);
		// MountPoint mp = null;
		// if ( mp == null ) {
		...
		String stagingFS = so.getCurrentSubmissionLocation()
				.getFirstStagingFileSystem(fqan);

		if (stagingFS == null) {
			myLogger
					.warn("Couldn't find staging filesystem for submission queue: "
							+ so.getCurrentSubmissionLocation().getLocation()
							+ ". Not setting anything.");
			return;
		}

		myLogger.debug(("Using staging filesystem: " + stagingFS
				+ " for submissionQueue: " + so.getCurrentSubmissionLocation()
				.getLocation()));
		templateNodes.get("ExecutionFileSystem").getTemplateNodeValueSetter()
				.setExternalSetValue(stagingFS);

		String[] modules = so.getCurrentModules();

		if (modules == null) {
			myLogger
					.error("Not setting anything. There seems to be an error somewhere.");
			return;
		}

		if (modules.length == 0) {
			templateNodes.get("Module").getTemplateNodeValueSetter()
					.setExternalSetValue("");
		} else {
			// TODO change that later for more modules
			templateNodes.get("Module").getTemplateNodeValueSetter()
					.setExternalSetValue(modules[0]);
		}
		...
		if (submissionObjectListener != null
			...
			while (e.hasMoreElements()) {
				SubmissionObjectListener l = (SubmissionObjectListener) e
						.nextElement();
