...
public class ValidatorFactory {
	...
	static final Logger myLogger = Logger.getLogger(ValidatorFactory.class
			.getName());

	public static TemplateNodeValidator createValidator(
			TemplateNode templateNode, String validatorName)
			...
			if (validatorName != null && !"".equals(validatorName)) {
				...
				if (validatorName.indexOf(".") == -1) {
					...
					validatorClass = Class
							.forName("org.vpac.grisu.client.model.template.validators."
									+ validatorName);
				} else {
					// means custom type
				...
				validator = (TemplateNodeValidator) validatorConstructor
						.newInstance(new Object[] { templateNode });
			}
		} catch (Exception e) {
			myLogger.warn("Could not create validator \"" + validatorName
					+ "\": " + e.getLocalizedMessage());
			throw new ValidatorNotCreatedException(
					"Could not create validator \"" + validatorName + "\": "
							+ e.getLocalizedMessage());
		}
