...
public class NamdConfigInputFile extends InputFile {
	...
	private void parseNamdInputFile() throws TemplatePreProcessorException {
        ...
        {
            throw new TemplatePreProcessorException((new StringBuilder()).append("Could not read namd config file: ").append(namd_config_file.toString()).toString());
        }
        ...
        int minimize = Integer.parseInt((String)propertiesToParse.get("minimize"));
        int run = 0;
        int numsteps = 0;
        ...
        if(propertiesToParse.get("numsteps") != null)
            numsteps = Integer.parseInt((String)propertiesToParse.get("numsteps"));
        int steps = minimize + run + numsteps;
        Map<String, String> result = new HashMap();
        node.getTemplate().getJobProperties().put("totalSteps", (new Integer(steps)).toString());
        node.getTemplate().getJobProperties().put("minimize", (new Integer(minimize)).toString());
        node.getTemplate().getJobProperties().put("numsteps", (new Integer(run + numsteps)).toString());
