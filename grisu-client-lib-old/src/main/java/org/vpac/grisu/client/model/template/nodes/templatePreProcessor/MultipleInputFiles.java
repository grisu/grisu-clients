...
public class MultipleInputFiles extends TemplatePreProcessor {
	...
	public void process() throws TemplatePreProcessorException {
		...
		InputFile.calculatePostProcessUploadAttribute(node.getElement()
				.getTextContent(), node.getElement());

		Element dataStagingElement = (Element) node.getElement()
		...
		for (int i = 1; i < fileUrls.length; i++) {
			...
			Element uriElement = ((Element) ((Element) newElement
			...
			InputFile.calculatePostProcessUploadAttribute(uriElement
					.getTextContent(), uriElement);
	...
	protected void resetTemplateNode() {
		...
		for (Element element : addedElements) {
			try {
				...
				// e.printStackTrace();
				myLogger
						.error("Error when trying to reset MultipleInputFiles: "
