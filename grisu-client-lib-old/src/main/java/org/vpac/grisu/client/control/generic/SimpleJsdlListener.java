...
public class SimpleJsdlListener implements JsdlTemplateListener {
	...
	public void submissionExceptionOccured(JsdlTemplateEvent event,
		...
		e = exception;
		System.err.println(event.getMessage()+": ["+exception.getLocalizedMessage()+"]");
		if ( exception.getParentException() != null ) {
			System.err.println(event.getMessage()+"Parent exception: "+exception.getParentException().getLocalizedMessage());
		}
