...
public class TemplateHelperUtils {
	...
	public static boolean checkValidParameterValue(
			String parameterNameWithOptionalConfig) {
	...
	public static Map<String, String> getConfig(
		...
		if (checkValidParameterValue(parameterNameWithOptionalConfig)) {
			...
			String config = parameterNameWithOptionalConfig.substring(
					start + 1, end);
			...
			for (String configOption : config.split(DELIMITER)) {
				...
				if (configOption.contains(KEY_VALUE_DELIMITER)) {
					...
					int indexDelimiter = configOption
							.indexOf(KEY_VALUE_DELIMITER);
			...
			throw new RuntimeException("Invalid configuration in template: "
					+ parameterNameWithOptionalConfig);
		}
	...
	public static String getParameterName(String parameterNameWithOptionalConfig) {
		...
		if (checkValidParameterValue(parameterNameWithOptionalConfig)) {
			...
			if (start == -1) {
				...
				String output = parameterNameWithOptionalConfig.substring(0,
						start);
				return output;
			}
