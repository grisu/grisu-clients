...
public class FileTransfer {
	...
	static final Logger myLogger = Logger.getLogger(FileTransfer.class
			.getName());
	...
	/**
	 * Use this constructor if you want to download a folder structure to the
	 * local machine.
	 * 
	 * @param target_folder
	 ...
	 */
	public FileTransfer(GrisuFileObject[] sources,
			GrisuFileObject targetDirectory, int overwrite_mode) {
	...
	private void download(boolean calledExternally,
			GrisuFileObject[] downloadSources) throws FailedDownloadsException {
		...
		for (GrisuFileObject remoteFile : downloadSources) {
			String[] childs = null;
			try {
				childs = em.getServiceInterface().getChildrenFileNames(
		...
		for (String child : allChilds) {
			try {
				...
				download(targetDirectory.getLocalRepresentation(false),
						source_folder, child, overwriteMode);
	...
	throws FailedDownloadsException {
		...
		if (isFolder) {
			...
			case OVERWRITE_EVERYTHING:
				try {
					downloadAndOverwrite(remoteFile, source_folder,
							target_folder);
				} catch (Exception e) {
					e.printStackTrace();
			...
			case OVERWRITE_ONLY_OLDER_FILES:
				try {
					downloadOnlyNewerFiles(remoteFile, source_folder,
							target_folder);
				} catch (Exception e) {
					e.printStackTrace();
					failed_downloads.put(remoteFile, e);
			...
			case DONT_OVERWRITE:
				try {
					downloadOnlyNonexistantFiles(remoteFile, source_folder,
							target_folder);
				} catch (Exception e) {
					e.printStackTrace();
					failed_downloads.put(remoteFile, e);
	...
	private void downloadAndOverwrite(String remoteFile, String source_folder,
			File target_folder) throws Exception {
	...
	private void downloadOnlyNewerFiles(String remoteFile,
	...
	private void downloadOnlyNonexistantFiles(String remoteFile,
			String source_folder, File target_folder) throws Exception {
	...
	public String getTransferMessages() {
		...
		for (int i = 0; i < status_messages.size(); i++) {
			...
			String time = sdf.format(status_messages.keySet().toArray(
					new Date[] {})[i]);
			result.append(time + ": "
					+ status_messages.values().toArray(new String[] {})[i]);
			result.append("\n");
		}
	...
	public void killTransfer() {
		...
		if ((transferThread != null) && transferThread.isAlive()) {
			...
			fireFileTransferEvent(new FileTransferEvent(this,
					FileTransferEvent.TRANSFER_CANCELLED));
		}
	}
		...
		transferThread = new Thread() {
			...
			public void run() {
				try {
					...
					fireFileTransferEvent(new FileTransferEvent(
							FileTransfer.this,
							FileTransferEvent.TRANSFER_STARTED));
					if (isDownload) {
						download(false, sources);
					} else {
						transferFiles();
					}
					myLogger.debug("File transfer finished for target: "
							+ targetDirectory.getName());
					status = FINISHED_STATUS;
					addStatusMessage("File transfer finished.");
					fireFileTransferEvent(new FileTransferEvent(
							FileTransfer.this,
							FileTransferEvent.TRANSFER_FINISHED));
				} catch (Exception e) {
					e.printStackTrace();
					...
					if (status == CANCELLED_STATUS) {
						...
						myLogger.debug("File transfer failed for target: "
								+ targetDirectory.getName());
						addStatusMessage("File transfer failed: "
	...
	private void transferFileFromLocalToLocal(GrisuFileObject sourceFile,
			GrisuFileObject targetDirectory) {
	...
	private void transferFileFromLocalToRemote(GrisuFileObject sourceFile,
		...
		if (sourceFile.getType() == FileConstants.TYPE_FOLDER) {
			...
			for (GrisuFileObject child : children) {
				...
				transferFileFromLocalToRemote(child, targetDirectory
						.getFileSystemBackend().getFileObject(newFolder));
			}
			...
			try {
				...
				em.getServiceInterface().upload(new DataHandler(source),
	...
	private void transferFileFromRemoteToLocal(GrisuFileObject sourceFile,
			GrisuFileObject targetDirectory) throws FileSystemException {
		try {
	...
	private void transferFileFromRemoteToRemote(GrisuFileObject sourceFile,
			GrisuFileObject targetDirectory) throws FileSystemException {
		try {
