...
public class InputFile extends TemplatePreProcessor {
	...
	public static void calculatePostProcessUploadAttribute(String url, Element element) {
	...
	public static void removePostProcessUploadAttribute(Element element) {
		...
		if ( currentpostprocess != null && !"".equals(currentpostprocess) ) {
			...
			if ( startIndex != -1 ) {
				...
				StringBuffer temp = new StringBuffer(currentpostprocess.substring(0, startIndex));
	...
	public static void addPostProcessUploadAttribute(Element element) {
			...
			if ( currentpostprocess == null || "".equals(currentpostprocess) ) {
				...
				//we add the upload postprocess to the existing one(s). we push it at front because most likely a substitute postprocess will follow
