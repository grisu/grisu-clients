...
/**
 * A TemplateProcessorFacory provides a simple factory method to create template preprocessors
 * via reflection. If you don't provide the full path to your preprocessor class it has to be
 * a class in the org.vpac.grisu.client.model.template.nodes.templatePreProcessor package.
 * 
	...
	public static TemplatePreProcessor createPreprocessor(TemplateNode node) {
		...
		try {
			...
			if ( node.getType().indexOf(".") == -1 ) {
				...
				processorClass = Class.forName("org.vpac.grisu.client.model.template.nodes.templatePreProcessor."+node.getType());
			} else {
				//means custom type
			...
			processor = (TemplatePreProcessor)processorConstructor.newInstance(new Object[]{ node });
			}
		} catch (Exception e) {
			myLogger.warn("Could not create template processor \""+node.getType()+"\": "+e.getLocalizedMessage());
			return null;
		}
