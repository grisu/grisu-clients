...
public class JsdlTemplate implements TemplateNodeListener {
	...
	/**
	 * Fires either a status event when the submission status changed or a
	 * message that the submission failed with the according
	 * {@link JobSubmissionException}
	 * 
	 * @param exception
	 *            the exception to pass or null if you only want to announce a
	 *            new status
	 */
	...
	/**
	 * Returns the number of postprocessors if the status of the submission >=
	 * STATUS_JOB_CREATED. Else -1.
	 * 
	 * @return the number of postprocessors of this template or -1 if they are
	 *         not yet parsed.
	 */
	 ...
	 * template. Only call this method after the status is
	 * STATUS_POSTPROCESSORS_EXECUTED - 1
	 * 
	...
	private List<ElementPostprocessor> getPostprocessors()
		...
		// throw new
		// JobSubmissionException("Postprocessors created before job is created on the server. Cancelling job sumbission.",
		// null);
		// }
	...
	// *
	// * @return the site if status > {@link #STATUS_VARIABLES_SUBSTITUTED},
	// else
	// * null
	// */
	 ...
	 * 
	 * @param deletePossiblyCreatedJob
	 *            deletes a job that may have been created remotely before
		...
		if (deletePossiblyCreatedJob) {
			if (this.status >= STATUS_JOB_CREATED
				...
				try {
					...
					try {
						em.getServiceInterface().deleteFile(
								currentRemoteJobDirectory);
					} catch (Exception e) {
						myLogger
					...
					// em.getJobManager().getJob(currentJobname);

					// em.getJobManager().cleanJob(job);
				} catch (Exception e) {
					myLogger.warn("Couldn't clear half-submitted job: "
							+ this.currentJobname);
				}
			}
		}
	...
	/**
	 * This is a convenience method. If your client just uses a default methods
	 * of setting values, you don't need specialised value setters. This methods
	 * adds a {@link DefaultTemplateNodeValueSetter} to every TemplateNode in
	 * this tmeplate.
	 */
	...
	// /**
	// * Gets the currently set relative job directory for this template
	// (relative
	...
	private void setStatus(int status) {
		...
		switch (status) {
		case STATUS_JSDL_CREATED:
			statusMessage = "Jsdl template created.";
			break;
		case STATUS_MODULES_CREATED:
			statusMessage = "Modules created.";
			break;
		case STATUS_TEMPLATENODES_CREATED:
			statusMessage = "TemplateNodes created.";
			break;
		case STATUS_SUBMISSION_STARTED:
			statusMessage = "Submission started.";
			break;
		case STATUS_TEMPLATE_NODES_FILLED:
			statusMessage = "TemplateNodes filled.";
			break;
		case STATUS_TEMPLATENODES_PROCESSED:
			statusMessage = "TemplateNodes processed.";
			break;
		case STATUS_JOB_CREATED:
			statusMessage = "Job created on server.";
			break;
		...
		case STATUS_POSTPROCESSORS_EXECUTED:
			...
			break;
		case STATUS_JOB_JSDL_DESCRIPTION_STORED:
			statusMessage = "Job description stored on server.";
			break;
		case STATUS_JOB_SUBMITTED:
			statusMessage = "Job submitted to endpoint.";
			break;
		case STATUS_JOB_PROPERTIES_STORED:
			statusMessage = "Additional job properties stored on server.";
			break;
		case STATUS_JOB_SUBMISSION_SUCCESSFUL:
			statusMessage = "Job submission successful.";
			break;
		default:
	 ...
	 * currentRemoteJobDirectory is the url to the actualy job (working)
	 * directory (as gridftp url). currentRelativeJobDirectory is the relative
	 * path to the working directory from the root of the
	 * executionhostFileSystem (aka MountPoint)<br>
	 * 
	 * 
	 * 
	 * @param fqan
	...
	private void setSubmissionLocation(String fqan)
		...
		// currentRemoteJobDirectory.substring(currentExecutionHostFileSystem.length()
		// + i);
	 ...
	 * @throws JobSubmissionException
	 *             if a submission using this template is already in progress
	 * 
	 * @throws JobSubmissionException
	...
	private void submit() throws JobSubmissionException {
		...
		try {
			...
			try {
				currentJobname = em.getServiceInterface().createJob(
			...
			throw new JobSubmissionException(
					"Unknown cause for job submission failure.",
					runtimeExceptionMostLikely);
		}
