...
public class CommandLine extends TemplatePreProcessor {
	...
	public static void main(String[] args) {
		...
		try {
			strings = CommandlineHelpers
					.parseString("cat  hallo \"what is  up\" fellow \" traveller?\"  ");
	...
	public void process() throws TemplatePreProcessorException {
		...
		if (strings.size() > 1) {
			...
			// we need to get the child element right after the Executable
			// element
	...
	protected void resetTemplateNode() {
		...
		for (Element element : addedElements) {
			try {
				...
				// e.printStackTrace();
				myLogger.error("Error when resetting template node: "
