...
public class JobManager {
	...
	static final Logger myLogger = Logger.getLogger(JobManager.class.getName());
	...
	public void cleanJob(GrisuJobMonitoringObject job)
	...
	public void cleanJobs(List<GrisuJobMonitoringObject> jobs)
			throws RemoteFileSystemException, NoSuchJobException {
		...
		JobHelper.cleanJobs(jobnames, em, ClientPropertiesManager
				.getConcurrentThreadsDefault());
	 ...
	 * 
	 * @param refresh
	 *            whether to refresh the status of every job or use a possible
	 *            cached state
	 * @return a list of all Jobs
	 */
	public synchronized EventList<GrisuJobMonitoringObject> getAllJobs(
			boolean refresh) {
	...
	public GrisuJobMonitoringObject getJob(String name)
		...
		throw new NoSuchJobException("No job with jobname \"" + name
				+ "\" in database.");
	}

	public GrisuJobMonitoringObject getJob(String name, boolean refreshJobList)
			throws NoSuchJobException {
		if (refreshJobList) {
			refreshAllJobs(true);
		}
	...
	public void killJobs(List<GrisuJobMonitoringObject> jobs)
			throws NoSuchJobException {
		...
		JobHelper.killJobs(jobnames, em, ClientPropertiesManager
				.getConcurrentThreadsDefault());
	 ...
	 * @throws NoSuchJobException
	 *             if the job doesn't exist in the databse
	 */
	...
	/**
	 * This can be used to check the status of jobs instead of implementing a
	 * loop on the list itself. It'll use threads soon so it should be faster...
	 * 
	 * @param jobs
	 *            the jobs to refresh
	 */
	 ...
	 * 
	 * @param jobnames
	 *            the names of the jobs to refresh
	 * @throws NoSuchJobException
	 *             if one of the specified jobnames doesn't exist. All jobnames
	 *             before the failed one were refreshed in that case.
	 */
	...
	private synchronized void retrieveAllJobs() {
		...
		String[] allJobnamesTemp = em.getServiceInterface()
		...
		//
		// allJobnames = new TreeSet<String>();
		// // EventList tempJobs = new
		// BasicEventList<GrisuJobMonitoringObject>();
		//
