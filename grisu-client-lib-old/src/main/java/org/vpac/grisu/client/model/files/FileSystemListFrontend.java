...
public class FileSystemListFrontend extends AbstractListModel {
	...
	static final Logger myLogger = Logger.getLogger(FileSystemListFrontend.class.getName());
	...
	public FileSystemListFrontend(FileSystemBackend fsb) {
		ApplicationStatusManager.getDefaultManager().setCurrentStatus("Connecting to filesystem: "+fsb.getAlias());

		this.fileSystemBackend = fsb;
		myLogger.debug("Creating frontend for filesystem backend: "+fsb.getRootUri().toString());
		
//		fileSystemBackend.getRoot().refresh();
		
	}
	
	public FileSystemListFrontend(FileSystemBackend fsb, GrisuFileObject virtualRoot) {
		this.fileSystemBackend = fsb;
//		fileSystemBackend.getRoot().refresh();
	...
	public void setCurrentDirectory(GrisuFileObject file) {
		if ( fileSystemBackend.isInFileSystem(file) ) {
			int index = getSize();
			if ( file.getURI().toString().startsWith(getVirtualRoot().getURI().toString()) ) {
	...
	private void fillCache() {
		...
		if ( list == null ) {
			myLogger.error("Could not list directory: "+getCurrentDirectory().getName());
			return;
		}
	...
	public Object getElementAt(int index) {
		...
		String[] keys = cachedCurrentDirectory.keySet().toArray(new String[cachedCurrentDirectory.keySet().size()]);
...
//		return ( currentDirectory.isRoot() || currentDirectory.equals(virtualRoot) );
	...
	public boolean equals (Object otherObject) {
		...
		if (otherObject instanceof FileSystemListFrontend) {
			...
			if ( other.getFileSystemBackend().equals(this.getFileSystemBackend()) &&
	...
	private String process(GrisuFileObject file) { 
		...
		case SORT_BY_TYPE_THEN_NAME:
			...
			if ( FileConstants.TYPE_FOLDER == file.getType() ) {
				...
				throw new RuntimeException("Filetype not file nor folder nor root. That's bad. Exiting...");
			}
		default: throw new RuntimeException("Sort mode not supported. Exiting...");
		}
