...
/**
 * GenericJobWrapper is supposed to be an easy-to-use wrapper class that
 * encapsulates functionality to create and submit a job using Grisu.
 * 
 * This class internally wrapps a {@link Common} module to take care of the
 * usual common job attributes.
 * 
 * Have a look at the main methods in {@link SimpleCommandlineClient} or
 * {@link SimpleCommandlineClientNoMds} to see examples on how it is used.
 * 
...
public class GenericJobWrapper {
	...
	static final Logger myLogger = Logger.getLogger(GenericJobWrapper.class
			.getName());
	...
	public static void main(String[] args) {
		...
		try {
			LoginParams loginparams = new LoginParams(
					"https://grisu.vpac.org/grisu-ws/services/grisu", "markus",
			...
			while (application == null || "".equals(application)) {
				System.out
						.println("Please enter the application you want to use. Press enter to see a list of all available applications.");
				...
				if (application == null || "".equals(application)) {
					...
					for (String name : em.getServiceInterface()
							.getAllAvailableApplications(
									DtoStringList.fromStringColletion(em
			...
			while (jobname == null || "".equals(jobname)) {
				System.out
						.println("Please enter the name of the job. Press enter to see a list of all your current jobnames.");
				jobname = in.readLine();
			...
			while (fqan == null || "".equals(fqan)) {
				System.out
						.println("Please enter the VO you want to use. Press enter to see a list of all your VOs.");
				fqan = in.readLine();
			...
			SubmissionLocation[] submissionLocations = job
					.getSubmissionLocations().toArray(
			...
			for (int i = 1; i < submissionLocations.length + 1; i++) {
				System.out.println("[" + i + "] "
						+ submissionLocations[i - 1].getLocation() + " ("
						+ submissionLocations[i - 1].getSite() + ")");
			}
			...
			String[] fqans = job.setSubmissionLocation(submissionLocation)
					.toArray(new String[] {});
			//			
			...
			System.out
					.println("Please select the executable you want to use: ");
	 ...
	 * job description. The base template is located here:
	 * src/main/resources/baseTemplate.xml.
	 * 
	 * @param em
	 ...
	 * to use your own one, provide it here. Your template has to use the
	 * {@link Common} module in order for this to work. Also, you have to have
	 * one template node with the name of the {@link #COMMANDLINE_TEMPLATE_NODE}
	 * value and one with the name of the {@link #INPUT_FILES_TEMPLATE_NODE}.
	 * You can have your own additional template nodes which you would have to
	 * take care of manually. Now that I think of it: it probably would be
	 * better, if you want to use your own template, to use this class as base
	 * class and write your own JobWrapper class that extends it.
	 * 
	 * @param em
	 *            the environment manager
	...
	public GenericJobWrapper(EnvironmentManager em, Document xmlTemplateDoc) {
		this.em = em;
		try {
			initialize(JsdlHelpers.getApplicationName(xmlTemplateDoc),
					xmlTemplateDoc);
		} catch (TemplateException e) {
			// should never happen. If it does, there's something major wrong
			// with the code/template
			e.printStackTrace();
		}
	}
	 ...
	 *            the environment manager
	 * @param application
	 *            the appliation used for job submission
	 */
	...
	/**
	 * Add an input file to the job at hand. Input files either are local (for
	 * example: "/home/markus/test.txt") or remote (like:
	 * gsiftp://ngdata.vpac.org/home/markus/test.txt). If they should be within
	 * the file structure of one of the users mountpoints. Didn't test yet what
	 ...
	 *             if this object is not initialized yet.
	 */
	public void addInputFile(String inputFileUrl) throws JobCreationException {
		...
		if (currentInputFiles == null || currentInputFiles.length() == 0) {
			...
			inputFilesSetter.setExternalSetValue(currentInputFiles + ";"
					+ inputFileUrl);
			myLogger.debug("New input files: " + currentInputFiles + ";"
					+ inputFileUrl);
		}
	}

	/**
	 * Since the actual job submission runs in it's own thread, you've got the
	 * opportunity here to connect a listener to figure out what's going on.
	 * 
	 * @param l
	 *            the listener
	 */
	...
	/**
	 * Change the type of application you want to use. Since you can reuse this
	 * object after a job submission finished, this makes sense in some cases.
	 * 
	...
	/**
	 * Use this method if a job submission failed. It will delete a possibly
	 * created remote job directory and remove the job handle from the database.
	 */
	...
	/**
	 * Returns all queues that are available for the specified site in
	 * combination with the currently selected VO and application.
	 * 
	 * @param site
	 *            the site
	...
	/**
	 * Returns all available sites for the currently selected application & VO
	 * for this user.
	 * 
	 * @return all available sites
	 */
	public Set<String> getCurrentlyAvailableSites() {
		return getCommonGenericModule().getAvailableSitesForCurrentVO()
				.keySet();
	}

	/**
	 * A convenience method to get a list of the names of all currently running
	 * jobs. Needed to figure out a jobname for a new job, since a jobname has
	 * to be unique.
	 * 
	 * @return all jobnames
	 */
	public String[] getCurrentlyExistingJobnames() {
		...
		for (GrisuJobMonitoringObject job : em.getJobManager()
				.getAllJobs(false)) {
	...
	/**
	 * If {@link #useMds(boolean)} is set to true and executables are published
	 * for the currently selected application and the currently selected
	 * submission location, the this returns all available executables for this
	 * situation.
	 * 
	 * If the internal state of this job wrapper object is inconclusive (aka
	 * either application or submission location is not set or they don't fit
	 * together) then the JobCreationException is thrown.
	 * 
	 ...
	 * @throws JobCreationException
	 *             if the internal state of this object is inconclusive
	 */
	public String[] getExecutablesForCurrentState() throws JobCreationException {
		return getCommonGenericModule().getCurrentSubmissionObject()
				.getCurrentExecutables();
	}

	/**
	 * Returns a list of all available submission locations for the this VO
	 * (which depends on whether {@link #useMds(boolean)} is set or and if so,
	 * which application is set).
	 * 
	 * @return all available submission locations for the current state (may be
	 *         empty)
	 */
	public Set<SubmissionLocation> getSubmissionLocations() {
		return getCommonGenericModule()
				.getAvailableSubmissionLocationsForCurrentVO();
	}
	...
	/**
	 * If {@link #useMds(boolean)} is set to true all available versions for the
	 * currently set application on the currently set submission location are
	 * returned.
	 * 
	...
	/**
	 * This method is the first one to call (if you didn't use the
	 * {@link #GenericJobWrapper(EnvironmentManager, String)} constructor). It
	 * initializes the job wrapper object and it's internal state to use the
	 * specified application.
	 * 
	 * Don't use this method to change the appliation. Use
	 * {@link #changeApplication(String)} for that.
	 * 
	 * @param application
	...
	public void initialize(String application) {
		...
		basetemplateStream = GenericJobWrapper.class
				.getResourceAsStream("/baseTemplate.xml");
	...
	private void initialize(String application, Document xmlTemplateDoc)
		...
		template.getTemplateNodes().get(COMMANDLINE_TEMPLATE_NODE)
				.setTemplateNodeValueSetter(commandLineSetter);
		template.getTemplateNodes().get(INPUT_FILES_TEMPLATE_NODE)
				.setTemplateNodeValueSetter(inputFilesSetter);

		TemplateNode stdoutNode = template.getTemplateNodes().get(
				STDOUT_TEMPLATE_NODE);
		TemplateNode stderrNode = template.getTemplateNodes().get(
				STDERR_TEMPLATE_NODE);
	...
	/**
	 * Resets the internal state of the job so the object can be reused again.
	 * This should give improved mds performance over creating a new one.
	 */
	public void reset() {
		try {
			template.reset(false);
		} catch (TemplatePreProcessorException e) {
			myLogger.error("Couldn't reset template: "
					+ e.getLocalizedMessage());
		}
	}

	/**
	 * Sets the commandline that is supposed to be executed by the job. For
	 * example, a simple cat job with one input file named test.txt would need:
	 * "cat test.txt". You also would have to add "test.txt" (of course with
	 * it's full path) using the {@link #addInputFile(String)} method.
	 * 
	 * @param commandLine
	...
	/**
	 * If you want to get notified when your job is finished/cancelled, specify
	 * your email address here. If not, don't use it or set null.
	 * 
	 * @param email
	 *            your email address
	 */
	...
	/**
	 * Much like {@link #addInputFile(String)}. Only files that were previously
	 * added are overwritten. Useful, for example if you called {@link #reset()}
	 * and don't want to use the files from the last submission.
	 * 
	 * @param inputFileUrl
	 *            the file
	 * @throws JobCreationException
	...
	/**
	 * Sets the name for this job. Internally, Grisu uses the jobname as a
	 * handle, so you need to specify a unique name here. To see what jobnames
	 * are in use at the moment, call {@link #getCurrentlyExistingJobnames()}. A
	 * jobname can be reused once the job is deleted.
	 * 
	 * @param jobname
	...
	/**
	 * Sets the module that should be loaded for this job on the remote
	 * resource. This overrides the calculated module for a mds enabled job
	 * until either a new VO or SubmissionLocaiton is set. For a java job you
	 * would set "java" here.
	 * 
	 * @param module
	 *            the module
	...
	/**
	 * Sets the submission location and returns a list of available VOs for the
	 * combination of selected application and submission location
	 * 
	 * @param subLoc
	 *            the submission location
	 * @return possible VOs that can be used with the current combination of
	 *         application and submission location
	 * @throws SubmissionLocationException
	 *             if the submission location is not available for this
	 *             application
	 * @throws JobCreationException
	 *             if the application is not set yet.
	 */
	public Set<String> setSubmissionLocation(SubmissionLocation subLoc)
			throws SubmissionLocationException, JobCreationException {
		getCommonGenericModule().setSubmissionLocation(subLoc);
		return getCommonGenericModule()
				.getAvailableVOsForCurrentSubmissionLocation();
	}

	/**
	 * Here you can set the version of the application you want to use for this
	 * job submission. It is recommended that you choose one of the values that
	 * are returned by {@link #getVersionsForCurrentState()} otherwise you'll
	 * get an exception.
	 * 
	 * @param version
	 *            the version to use.
	 * @throws JobCreationException
	 *             if the version you choose is not supported at the currently
	...
	/**
	 * Sets the current VO and returns a list of all available submission
	 * locations for the this VO (which depends on whether
	 * {@link #useMds(boolean)} is set or and if so, which application is set).
	 * 
	 * @param fqan
	 *            the VO
	 * @return all available submission locations for the current state (may be
	 ...
	 *             if the application is not set yet.
	 */
	public Set<SubmissionLocation> setVO(String fqan)
			throws JobCreationException {
	...
	/**
	 * Starts the submission. The submission runs in it's own thread, so you
	 * might want to connect a listener (
	 * {@link #addJsdlTemplateListener(JsdlTemplateListener)} to monitor the
	 * progress (of the submission -- not the actual job).
	 * 
	 * @throws JobSubmissionException
	 *             if some basic requirements are not fullfilled the job
	 *             submission doesn't start in the first place.
	 */
	...
	/**
	 * Switch between using mds for application specific lookups or not. If you
	 * specify true, the object will try to find all submission locations that
	 * are available for the selected VO and the mountpoints a user has got
	 * mounted. It won't display queues of sites where the application is not
	 * installed for example.
	 * 
	 * If you specify false, Grisu still uses a bit of mds information. But only
	 * to find out which submission locations are available for the user
	 * (according to currently set VO and available mountpoints). The type of
	 * application is not considered here. This is useful if you can't rely on
	 * your application being published.
	 * 
	 * @param useMds
	...
	/**
	 * Since the actual job submission runs in it's own thread, here's a
	 * convenience method to wait until the submission is finished.
	 */
