...
public class UnderworldHelpers {
	...
	static final Logger myLogger = Logger.getLogger(UnderworldHelpers.class
			.getName());
	...
	public static String getMaxTimesteps(Document inputXmlFile) {
		...
		try {
			resultNodes = (NodeList) xpath.evaluate(expression, inputXmlFile,
					XPathConstants.NODESET);
		} catch (XPathExpressionException e) {
			myLogger
					.warn("No output directory information in underworld xml file.");
			e.printStackTrace();
			return null;
		}
