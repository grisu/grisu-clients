...
public class GridBlenderCheck implements BlenderMode {
			...
			try {
				...
				if ( downloadDir.exists() ) {
					if ( ! downloadDir.canWrite() ) {
						System.out.println("Can't write to specified output directory "+downloadDir.toString()+".");
						System.exit(1);
					}
				}
			} catch (Exception e) {
				System.out.println("Could not access specified download directory.");
			}
		}
	...
	public void execute() {
		...
		if (commandlineArgs.isLoopUntilFinished()) {
			...
			while ( ! blenderMultiPartJobObject.isFinished(false) ) {
				
				if ( sleepTime/60000 == 1 ) {
					System.out.println("Sleeping for "+sleepTime/60000+" minute.");
				} else {
					System.out.println("Sleeping for "+sleepTime/60000+" minutes.");
				}
			...
			if ( blenderMultiPartJobObject.isFinished(false) ) {
				...
				if ( commandlineArgs.isDownloadResults() ) {
					...
					String pattern = blenderMultiPartJobObject.getJobProperty(GrisuBlenderJob.BLENDER_OUTPUTFILENAME_KEY);
					if ( StringUtils.isBlank(pattern) ) {
						System.out.println("Could not determine output filename. Exiting...");
						System.exit(1);
					}
					String[] patterns = new String[]{pattern};
					try {
						System.out.println("Downloading output files that match \""+pattern+"\".\n");
						blenderMultiPartJobObject.downloadResults(downloadDirectory, patterns, false, false);
						System.out.println("Downloads finished.");
					} catch (Exception e) {
						System.out.println("Could not download results: "+e.getLocalizedMessage());
						System.exit(1);
					}
				}
			}
